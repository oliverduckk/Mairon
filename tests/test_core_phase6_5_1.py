...
def run():
    ...
    # --------------------------------------------------
    # 8. Resolved referents survive BOTH structured and
    #    transitional rendered-text contract transport.
    # --------------------------------------------------
    ...
    order_turn = classify_turn(
        "Hey has my order from Hype DC arrived?",
        conversation_state=conversation_state,
    )

    conversation_state.update_from_turn(
        order_turn
    )

    share_turn = classify_turn(
        (
            "They are a pair of XT6s for my trip to China "
            "in 2 months time."
        ),
        conversation_state=conversation_state,
    )
    ...
    share_turn = (
        conversation_state.resolve_follow_up(
            share_turn
        )
    )

    assert (
        share_turn.resolved_referents[
            "they"
        ].lower()
        == "hype dc order"
    )
    ...
    assert (
        share_contract.resolved_referents[
            "they"
        ].lower()
        == "hype dc order"
    )
    ...
    rendered_contract = (
        share_contract.to_model_instruction()
    )

    assert (
        "'they' refers to: Hype DC order"
        in rendered_contract
    )
    ...
    assert (
        structured_runtime.resolved_referents[
            "they"
        ].lower()
        == "hype dc order"
    )
    ...
    assert (
        legacy_runtime.resolved_referents[
            "they"
        ].lower()
        == "hype dc order"
    )
